...
class SettingsView(GradientFrame):
    def __init__(self, parent: tk.Widget, app: "ElementGachaApp") -> None:
        ...
        self.title_label = tk.Label(root, font=(config.FONT_ZH, 22, "bold"), bg=root_bg, fg="#111111")
        self.title_label.pack(pady=8)

        self.unlock_frame = tk.LabelFrame(root, padx=12, pady=12, bg=root_bg, fg="#111111")
        self.unlock_frame.pack(fill="x", padx=26, pady=8)

        row = tk.Frame(self.unlock_frame, bg=root_bg)
        row.pack(fill="x")

        self.unlock_entry = tk.Entry(row, width=32, font=(config.FONT_EN, 10))
        ...
        self.drop_hint_label = tk.Label(self.unlock_frame, text="", font=(config.FONT_ZH, 10), bg=root_bg, fg="#555555")
        ...
        self.unlock_status_label = tk.Label(self.unlock_frame, text="", font=(config.FONT_ZH, 11), bg=root_bg, fg="#111111")
        self.unlock_status_label.pack(anchor="w", pady=(8, 0))

        self.language_frame = tk.LabelFrame(root, padx=12, pady=12, bg=root_bg, fg="#111111")
        ...
        self.language_option["menu"].configure(font=(config.FONT_ZH, 10))
        ...
        self.save_frame = tk.LabelFrame(root, padx=12, pady=12, bg=root_bg, fg="#111111")
        ...
        self.message_label = tk.Label(root, text="", fg="#225b27", bg=root_bg, font=(config.FONT_ZH, 11))
    ...
    def apply_language(self) -> None:
        ...
        self.message_label.config(text=self.app.tr("settings_language_changed"), fg="#225b27")
    ...
    def apply_unlock_code(self) -> None:
        ...
        self.refresh_unlock_status()
        if paid_unlocked:
            self.message_label.config(text=f"{self.app.tr('settings_unlock_success')} (license:{reason})", fg="#225b27")
        else:
            self.message_label.config(text=f"{self.app.tr('settings_unlock_wrong')} (license:{reason})", fg="#8a2c2c")
    ...
    def clear_save_with_confirm(self) -> None:
        ...
        self.refresh_unlock_status()
        self.message_label.config(text=self.app.tr("settings_save_cleared"), fg="#225b27")
    ...
    def export_save_file(self) -> None:
        ...
        self.message_label.config(text=self.app.tr("settings_export_done", path=path), fg="#225b27")
    ...
    def import_save_file(self) -> None:
        ...
        try:
            ...
            self.refresh_unlock_status()
            self.message_label.config(text="已匯入存檔", fg="#225b27")
        except Exception:
            self.message_label.config(text="匯入失敗", fg="#8a2c2c")
